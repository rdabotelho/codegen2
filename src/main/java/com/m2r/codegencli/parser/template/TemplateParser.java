...
import com.m2r.codegencli.parser.TokenIterator;
...
public class TemplateParser extends Parser<Template> {
    ...
    public static Template parse(Reader reader) throws ParserException {
        TemplateParser me = new TemplateParser();
    ...
    private void updateTemplateDefinition(TemplateParser templateParser) {
            ...
            if (tokens.nextEqual("{")) {
                ...
                while (tokens.getLast() != null && !tokens.lastEqual("}")) {
                    if (tokens.getLast().isFlag(ATTRIBUTE)) {
                        ...
                        templateParser.result.getMethods().add(extractMethod(tokens));
                    }
                }
            }
        }
    ...
    private Method extractMethod(TokenIterator tokens) {
        Method method = new Method();
        method.setName(tokens.lastAsString());
        ...
        if (tokens.lastEqual("{")) {
            ...
            while (tokens.getLast() != null && !tokens.lastEqual("}")) {
                if (tokens.getLast().isFlag(METHOD)) {
                    method.getMethods().add(extractMethod(tokens));
                }
            }
            tokens.next();
        }
